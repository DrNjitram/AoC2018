    ...
    def play(self, elf_death=False):
        ...
        while True:
            if self.round(elf_death=elf_death):
                break
            rounds += 1
        return rounds, sum(unit.hp for unit in self.units if unit.alive), rounds * sum(unit.hp for unit in self.units if unit.alive)
...
lines = open('G:\\24daysofcode\\15_input').read().splitlines()
